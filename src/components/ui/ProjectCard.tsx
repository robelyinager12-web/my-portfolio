    ...
    <article className="overflow-hidden rounded-[14px] border border-line bg-paper-3 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_24px_44px_-22px_rgba(21,23,27,0.28)]">
      ...
      >
        <svg viewBox="0 0 300 160" className="absolute inset-0 h-full w-full opacity-55">
          <polygon
            points="40,20 120,40 90,110 20,90"
            fill="none"
            stroke={project.accentFrom}
            strokeWidth={1}
          />
          <polygon
            points="160,15 260,35 230,120 150,100"
            fill="none"
      ...
      <div className="px-[22px] pb-6 pt-[22px]">
        ...
        <div className="mb-[18px] flex flex-wrap gap-[7px]">
          {project.tags.map((tag) => (
            <span
              key={tag}
              className="rounded-[5px] border border-line px-2 py-1 font-mono text-[11px] text-ink-3"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="flex gap-[18px]">
            ...
            className="flex items-center gap-1 font-mono text-[12.5px] text-indigo-deep hover:underline"
          >
            Live demo →
          </a>
